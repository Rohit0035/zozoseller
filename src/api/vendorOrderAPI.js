...
export const UpdateVendorOrderPackageDetails = async (data) => {
  try {
    const response = await fetchWithAuth(
      `${API_CONFIG.baseURL}/vendor-orders/update-vendor-order-package-details/`,'POST',data
    );
    return response.data; // Axios automatically parses JSON
  } catch (error) {
    // throw new Error(error?.message || "Login failed");
...
export const generateShippingLabels = async (data) => {
  try {
    const response = await fetchWithAuth(
      `${API_CONFIG.baseURL}/vendor-orders/generate-shipping-labels/`,'POST',data
    );
    return response.data; // Axios automatically parses JSON
  } catch (error) {
    // throw new Error(error?.message || "Login failed");
