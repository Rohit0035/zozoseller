...
const CancellationsAndReturns = () => {
  ...
  const [filteredCancelOrders, setFilteredCancelOrders] = useState([]);
  ...
  const [logisticsReturns, setLogisticsReturns] = useState([]);
  const [filteredLogisticsReturns, setFilteredLogisticsReturns] = useState([]);
  ...
  const fetchOrders = async (data) => {
        ...
        try {
          ...
          if (response.success == true) {
            ...
            const formattedCancelData = response.data.map((item, index) => ({
                index: index + 1,
                id: item._id,
                orderId: item.orderId?.orderUniqueId || 'N/A',
                ...
                amount: item.orderItems[0]?.total || 'N/A',
                reason: item.returnReason || 'N/A',
                date: item.returnDate || 'N/A',
                status: item.returnStatus || 'N/A', 
                refundStatus: item.refundStatus || 'N/A',
                channel: item.channel || 'N/A',
            }));
            const formattedReturnData = response.data.map((item, index) => ({
                index: index + 1,
                id: item._id,
                returnId: item.orderId?.returnId || 'N/A',
                orderId: item.orderId?.orderUniqueId || 'N/A',
                customerName: item.orderId?.userId?.name || 'N/A',
                contact: item.orderId?.userId?.phone || 'N/A',
                product: item.orderItems[0]?.productId?.name || 'N/A',
                ...
                logisticsPartner: item.logisticsPartner || 'N/A',
                trackingNumber: item.trackingNumber || 'N/A',
                returnStatus: item.returnStatus || 'N/A',
                refundStatus: item.refundStatus || 'N/A',
            }));
            setCancelOrders(formattedCancelData)
            setFilteredCancelOrders(formattedCancelData)

            setLogisticsReturns(formattedReturnData)
            setFilteredLogisticsReturns(formattedReturnData)
